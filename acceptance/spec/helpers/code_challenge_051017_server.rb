...
class CodeChallenge051017Server
  ...
  def start
    ...
    unless artifact_exists?
      ...
      puts '<'*80
      puts ''
      puts "No artifact found, create with 'cd ~/workspace/CodeChallenge051017/ && ./gradlew clean assemble'"
      puts ''
      puts '>'*80
      exit!
    end
    ...
    @service_offering_groups_app_pid = Process.spawn(
        "SERVER_PORT=#{app_port} SIMULATOR_PORT=#{simulator_port} java #{java_opts} -jar build/libs/CodeChallenge051017.jar",
        chdir: '../',
        pgroup: true,
  ...
  def artifact_exists?
    artifact_found = `[ -e ../build/libs/CodeChallenge051017.jar ] && echo true || echo false`
